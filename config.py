...
# 讲台前景像素占比阈值（低于此值 → 进入静止补偿判定）
TEACHER_PRESENCE_THRESHOLD = 0.02

# 静止补偿：当教师几乎不动时，使用人体检测兜底避免误判缺席
TEACHER_STATIC_COMPENSATION_ENABLED = True
TEACHER_STATIC_DETECT_INTERVAL = 2      # 每 N 个采样点做一次人体检测（其余复用上次结果）
TEACHER_STATIC_MIN_WEIGHT = 0.20        # OpenCV HOG 行人检测最小置信权重
TEACHER_STATIC_MIN_AREA_RATIO = 0.015   # 候选人体框面积占讲台区域最小比例
